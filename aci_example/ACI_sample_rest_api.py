...
req = requests.post(AUTH_URL, json={
    "aaaUser": {"attributes": {"name": LOGIN, "pwd": PASSWORD}}},
    verify=False)
token = req.json()["imdata"][0]["aaaLogin"]["attributes"]["token"]
cookie = {'APIC-cookie': token}

QUERY_URL = URL + \
    '/api/node/class/fvTenant.json?query-target-filter=eq(fvTenant.name,"Heroes")'
